	...
	elseif (pVoteCastPlayer:GetID() == Game.GetActivePlayer()) then
		...
		if (Teams[Game.GetActiveTeam()]:IsHasMet(iTeam)) then
			if (pTeam:IsMinorCiv()) then	
				strVoteCastTT = Locale.ConvertTextKey("TXT_KEY_POP_UN_ELEC_VOTE_TT_CITYSTATE_ALT", pPlayer:GetCivilizationShortDescription(), pVoteCastPlayer:GetCivilizationShortDescription());
			else
				strVoteCastTT = Locale.ConvertTextKey("TXT_KEY_POP_UN_ELEC_VOTE_TT_CIV_ALT", pPlayer:GetCivilizationShortDescription(), pVoteCastPlayer:GetCivilizationShortDescription());
			end
		end
...
function OnPopup( popupInfo )
	...
	local iVotes;
	local iBestVotes = -1;
	local iBestVotePlayer = -1;
	local iNumBestVotes = 1;
	
	local tTeams = {};
	
	-- Loop through all players
	for iPlayerLoop = 0, 12, 1 do
		...
		if (pLoopTeam:IsAlive()) then
			...
			if (iVotes > iBestVotes) then
				iBestVotes = iVotes;
				iBestVotePlayer = iPlayerLoop;
				iNumBestVotes = 1;
			elseif (iVotes == iBestVotes) then
				iNumBestVotes = iNumBestVotes + 1;
				iBestVotePlayer = -1;
			end
	...
	local strVotesToWinString;
	if (iBestVotePlayer > -1) then	
		...
		Players[iBestVotePlayer]:ChangeScoreFromFutureTech(250);
        	...
        	strVotesToWinString = Locale.ConvertTextKey("TXT_KEY_MEDIEVAL_SCENARIO_NO_HRE_WINNER");
	end
	...
	-- Don't show the popup if it is the preliminary election
	if (bPreliminaryElection) then
		OnClose();
	end
end
