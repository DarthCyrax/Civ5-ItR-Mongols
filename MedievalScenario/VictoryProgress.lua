...
-- For scenario
local g_savedData = Modding.OpenSaveData();
...
function PopulateDiplomatic()
	...
	if(PreGame.IsVictory(GameInfo.Victories["VICTORY_DIPLOMATIC"].ID))then
		...
		Controls.VotesHave:SetText(GetProjectedVotesForTeam(curTeam));
...
function PopulateDiploScreen()
	...
	table.sort(sortedPlayerList, function(a, b) return GetProjectedVotesForTeam(Players[b]:GetTeam()) <
														GetProjectedVotesForTeam(Players[a]:GetTeam()) end);
													   
	local iVaticanExtraVoteTeam = g_savedData.GetValue("VaticanExtraVoteTeam");
	if (iVaticanExtraVoteTeam == nil) then
		iVaticanExtraVoteTeam = -1;
	end
	...
	for i, v in ipairs(sortedPlayerList) do	
		...
		local iProjectedVotes = GetProjectedVotesForTeam(iTeam);
	...
	local totalVotes = 1; -- Start with 1, since Vatican City gives an extra
	for i, v in pairs(Teams) do
	...
	Controls.NeededVotes:SetText(Locale.ConvertTextKey("TXT_KEY_VP_DIPLO_VOTES_NEEDED"));
...
-- For scenario
function GetProjectedVotesForTeam(iTeam)
	local iVotes = 0;
	local pTeam = Teams[iTeam];
	local iVaticanExtraVoteTeam = g_savedData.GetValue("VaticanExtraVoteTeam");
	if (iVaticanExtraVoteTeam == nil) then
	...
	if (pTeam ~= nil) then
		iVotes = iVotes + pTeam:GetTotalProjectedVotes();
		if (iTeam == iVaticanExtraVoteTeam) then
			iVotes = iVotes + 1;
		end
	end
