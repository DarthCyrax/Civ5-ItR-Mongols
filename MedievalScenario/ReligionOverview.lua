...
function GetReligiousStatus()
	...
	local eReligion = GetPlayerReligionForScenario(iPlayer);
	local eFounderBenefitsReligion = Game.GetFounderBenefitsReligion(iPlayer);

	if (eReligion == eFounderBenefitsReligion) then
		text = Locale.Lookup("TXT_KEY_MEDIEVAL_SCENARIO_HAVE_FOUNDER_BELIEFS");
	else
		text = Locale.Lookup("TXT_KEY_MEDIEVAL_SCENARIO_NEED_FOUNDER_BELIEFS");
	end
...
function GetPlayerReligionForScenario(ePlayer)
...
function RefreshYourReligion()
	...
	-- don't do anything until all religions initialized
	if (Game.GetNumReligionsFounded() < 3) then
		return;
	end
	...
	local eFounderReligion = Game.GetFounderBenefitsReligion(iActivePlayer);
	local eReligion = GetPlayerReligionForScenario(iActivePlayer);
...
-----------------------------------------------------------------
-- Add Religion Overview to Dropdown
-----------------------------------------------------------------
